...
from jaxdecomp import transposeZtoY  # yapf: disable
...
def create_spmd_array(global_shape, pdims):
  ...
  local_array = jnp.arange(
      (global_shape[0] // pdims[1]) * (global_shape[1] // pdims[0]) * global_shape[2])

  local_array = local_array.reshape(global_shape[0] // pdims[1], global_shape[1] // pdims[0],
                                    global_shape[2])
  ...
  global_array = multihost_utils.host_local_array_to_global_array(local_array, mesh, P('z', 'y'))
...
global_shapes = [(4, 8, 16), (4, 4, 4),
                 (29 * size, 19 * size, 17 * size)]  # Cubes, non-cubes and primes
...
def test_tranpose(pdims, global_shape):
  ...
  gathered_jd_xy = multihost_utils.process_allgather(jd_tranposed_xy, tiled=True)
  gathered_jd_yz = multihost_utils.process_allgather(jd_tranposed_yz, tiled=True)
  gathered_jd_zy = multihost_utils.process_allgather(jd_tranposed_zy, tiled=True)
  gathered_jd_yx = multihost_utils.process_allgather(jd_tranposed_yx, tiled=True)

  gathered_jax_xy = multihost_utils.process_allgather(jax_transposed_xy, tiled=True)
  gathered_jax_yz = multihost_utils.process_allgather(jax_tranposed_yz, tiled=True)
  gathered_jax_zy = multihost_utils.process_allgather(jax_tranposed_zy, tiled=True)
  gathered_jax_yx = multihost_utils.process_allgather(jax_tranposed_yx, tiled=True)
  ...
  assert_array_equal(gathered_array.transpose(forward_tranpose), gathered_jax_xy)
  # *********************************************
  ...
  assert_array_equal(gathered_jax_xy.transpose(forward_tranpose), gathered_jax_yz)
  ...
  assert_array_equal(gathered_array.transpose(double_forward), gathered_jax_yz)
  # *********************************************
  ...
  assert_array_equal(gathered_jax_yz.transpose(backward_tranpose), gathered_jax_zy)
  ...
  assert_array_equal(gathered_jax_zy.transpose(backward_tranpose), gathered_jax_yx)
