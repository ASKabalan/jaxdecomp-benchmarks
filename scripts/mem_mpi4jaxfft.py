...
def normal(key, shape, comms=None):
    ...
    print(shape)
    return jax.random.normal(key, [shape[0] // nx, shape[1] // ny] +
                             list(shape[2:]))


def run_benchmark(global_shape, nb_nodes, pdims, precision, iterations,
                  output_path):
    """ Run the benchmark
    """
...
if __name__ == "__main__":
    ...
    if args.local_shape is not None:
        global_shape = (args.global_shape * jax.device_count(),
                        args.global_shape * jax.device_count(),
